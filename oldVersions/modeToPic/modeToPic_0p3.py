...
def mode2pic(fileIn, save_presets = {}, plot_presets = {}, saveOuts=1, \
             matrices=["EsqOut","nZ"],imageMeta = {}):
  '''
  ...
  if inFn.lower() in ("matdata.txt","rawdata.txt"):
     rD = getRD(fileIn)
     for keye in rD.keys():
       if keye in matrices:
         rD[keye] = np.transpose(np.reshape(rD[keye],\
                       [len(rD['xMat']),len(rD['yMat'])]))
         np.save(tryUntilGood(os.getcwd() + '\\', \
                              fileIn.rpartition('.')[0] + "_" + keye),rD[keye])
         ...
         if saveOuts:
           fileOut = tryUntilGood(os.getcwd() + '\\', \
                                  fileIn.rpartition('.')[0] + ".png")
           if inFn.lower() == "matdata.txt":
             plt.savefig(fileOut,metadata=imageMeta)
           if inFn.lower() == "rawdata.txt":
             plt.imsave(fileOut,newMat,**save_presets)
       #end if
     #end for
  #end if
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~main~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if __name__ == "__main__":
  ...
  for fileA in range(len(filesToDo)):
    print("converting " + filesToDo[fileA])
    ...
    mode2pic(filesToDo[fileA],plot_presets=plot_p,save_presets=save_p,saveOuts=1,\
             matrices=["EsqOut","nZ"],imageMeta=imageMetadata)
    if wincmd:
