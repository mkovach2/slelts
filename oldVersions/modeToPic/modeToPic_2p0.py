...
Created: 2022-06-22
Modified: 2022-06-23 miles @ hyperlightcorp.com
      ...
      if keye in matrices:
        ...
        if saveOuts:
          ...
          if (hasDouble):
            ...
            #end if outputTreatment == 4
            if not(outputTreatment in ["1","2","4",1,2,4]):
              outputTreatment = "0"
              saveOuts = 0
            #end if not(outputTreatment in ["1","2",1,2]) 
          #end if (hasDouble)
        ...
        if saveOuts:
          ...
          if (hasDouble):
            ...
            #end if outputTreatment == 4
            if not(outputTreatment in ["1","2","4",1,2,4]):
              print("outputTreatment in [\"1\",\"2\",\"4\",1,2,4]" + str(outputTreatment in ["1","2","4",1,2,4]))
              outputTreatment = "0"
              saveOuts = 0
            #end if not(outputTreatment in ["1","2",1,2])
...
if __name__ == "__main__":
  ...
  if pathToData[-4:] == ".txt":
    ...
    os.chdir(pathToData)
    #if pathToData.rsplit('.',1)[1] != "gudge":
    filesToDo = []
