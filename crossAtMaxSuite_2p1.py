...
def d_1d(vec1):
  # 1-d derivative
  return vec1[1:]-vec1[0:-1]
#end def diff(mat1,mat2)

def npClosest(inArr,inVal):
  # sometimes np.where just needs a little bit of help.
  # this function just finds where the closest value to inVal is,
  # in case the exact value doesnt show up in the array.
  diffArr = abs(inArr-inVal)
  locAcc = np.array(np.where(diffArr == np.min(diffArr)))
  if type(locAcc[0]) in (int,float,np.float32,np.float64):
    return np.array([locAcc])
  #end if type(locAcc[0]) in (int,float,np.float32,np.float64)
  elif type(locAcc[0]) == np.ndarray:
    deletey = []
    for entry in range(len(locAcc)):
      if np.size(locAcc[entry]) > 1 and not(np.any(locAcc[entry])):
        deletey.append(entry)
      #end if np.size(entry) > 1 and not(np.any(entry))
    #end for entry in locAcc
    locAcc = np.delete(locAcc,deletey,axis=0)
    return np.squeeze(locAcc)
  #end elif type(locAcc[0]) == np.ndarray
#end def npWhere()
...
def getMfd(lumNp,xax,yax,fudgeFactor = 200):
  '''
    fudgeFactor # maximum number of units above y=0 to be an upper bound
    # when finding max value. set to negative to find max
    # in BOx
    # enter fudgeFactor = ridge height for max in ridge
  '''
  
  fudgeFactor = fudgeFactor # maximum number of units above y=0 to be an upper bound
                  # when finding max value. set to negative to find max
                  # in BOx
                  # enter fudgeFactor = ridge height for max in ridge
  boxBound = np.where(np.abs(yax)==np.min(np.abs(yax)))[0][0] + fudgeFactor
  
  # where y = 0, ie the oxide's maximum height
  maxLo = np.max(lumNp[0:boxBound,:])
  maxLoAddrs = np.squeeze(np.where(lumNp[0:boxBound,:] == maxLo))
  ...
  if fudgeFactor < 0:
    ...
    xMaxLo = np.round(np.where(np.abs(xax)==np.min(np.abs(xax))))[0]
  #end else
  
  lowRow = lumNp[yMaxLo,:]
  lowCol = lumNp[:,xMaxLo]
  
  # find nearest local max to x=0
  locExt = np.multiply(abs(d_1d(lowRow)) < 0.0008,np.arange(len(lowRow[:-1])))
  xMaxLo = locExt[npClosest(xax,0)]
  maxLo = lowRow[xMaxLo]
  
  xPoints = [0,0]
  xPoints[0] = int(np.mean(npClosest(np.abs(lowRow[:xMaxLo-1]-maxLo/euler**2),0)))
  xPoints[1] = int(np.mean(npClosest(np.abs(lowRow[xMaxLo+1:]-maxLo/euler**2),0))) + xMaxLo + 1
  
  yPoints = [0,0]
  yPoints[0] = int(np.mean(npClosest(np.abs(lowCol[:yMaxLo-1]-maxLo/euler**2),0)))
  yPoints[1] = int(np.mean(npClosest(np.abs(lowCol[yMaxLo+1:]-maxLo/euler**2),0))) + yMaxLo + 1

  xMfd = xax[xPoints[1]]-xax[xPoints[0]]
  yMfd = yax[yPoints[1]]-yax[yPoints[0]]
  ...
  return mfd_dict
#end def getMfd(lumNp,xax,yax,fudgeFactor = 200)


def mf_contour(npy_in,num_of_contours = 1,thiccPx = 1,expand = 0):
  ...
  for contLevel in np.arange(1,num_of_contours + 1,1):
    truthMat = (lumNp >= maxLo/(contLevel * euler**2))
    # a contour which separates the MFD inside from outside
    
    if thiccPx > 1: #thicc version
      tMy = (truthMat[:-thiccPx,:] != truthMat[thiccPx:,:])
      tMx = (truthMat[:,:-thiccPx] != truthMat[:,thiccPx:])
      tMd = np.zeros(np.shape(truthMat))
      tMd[(thiccPx-1):-1,1:-(thiccPx-1)] = tMx[:-thiccPx,:] + tMy[:,:-thiccPx] + tMx[thiccPx:,:] + tMy[:,thiccPx:]
...
def niceTile(newMat,longX,longY,plotPresets = {},grid=1):
  ...
  knork = plt.figure(num=figNo,figsize=(hpix/dpi_,vpix/dpi_),dpi=dpi_)
  ax = plt.subplot()  
...
def mode2pic(fileIn, save_presets = {}, plot_presets = {},crop_um = (None,None),
  ...
  if inFn.lower() in ("matdata.txt","rawdata.txt","lumdat.txt") or fileIn[-4:] == '.npy':
    # "lumdat" is a legacy suffix. dont name anything new lumdat.
    
    if fileIn[-4:] == '.npy':
      shortName = fileIn.rpartition('.')[0]
      fileX = shortName + "_xMat" + ".npy"
      fileY = shortName + "_yMat" + ".npy"
    
      newMat = np.load(fileIn)
      longX = np.load(fileX)
      longY = np.load(fileY)
      ...
      if stretch and type(fileIn) != list:
        (newMat,lel,longX) = stretchMat(newMat,longX,axis_=cols)
        (newMat,lel,longY) = stretchMat(newMat,longY,axis_=rows)
      #end if expand
    #end if fileIn[-4:] == '.npy'
    ...
    xCroppy = np.arange(np.shape(newMat)[1])
    yCroppy = np.arange(np.shape(newMat)[0])
    ...
    (nTn,ax,knork) = niceTile(newMat,longX,longY,plot_presets)
    ...
    if contour and (inFn.lower() == "rawdata.txt" or fileIn[-4:] == '.npy'):
      ring1 = mf_contour(
        [newMat,longX,longY],
        **contour_presets
      )#end mf_contour
