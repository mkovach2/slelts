...
Created: 2022-06-22
Modified: 2023-02-10 miles @ hyperlightcorp.com
...
import crossAtMaxSuite_1p4 as cams
# includes functions needed to support this program.
# do not use any versions older than 1p2; they use a lot of RAM
...
if __name__ == "__main__":
  ...
  fromCmd = 0
  if len(sys.argv) == 2: #mostly for calling from other scripts
    pathToData = sys.argv[1]
    fromCmd = 1
  #end if len(sys.argv) == 2
  elif 1: # debugging_if, kind of
  ...
  showPlots = not(fromCmd)
  filesToDo = mDfiles + filesToDo
  # if len(filesToDo) > 5 and not(fromCmd):
  ...
  # for fileA in [0]:
    if filesToDo[fileA] in mDfiles:
      plot_p['vmin'] = 1
      plot_p['vmax'] = 4
      # ^ setting the max value of the color map to 3, so that all index plots 
      # are comparable to each other, so long as nothing we use has n > 4 lel
      plot_p['cmap'] = 'turbo'
      save_p['cmap'] = 'turbo' # if we're already not going to match any existing
        #software on this one, i can do what i want
    #end if fileA in mDfiles
    else:
      plot_p['vmin'] = 0
      plot_p['vmax'] = None
      plot_p['cmap'] = colorScheme
      save_p['cmap'] = colorScheme
    #end else
    ...
    if not(os.path.exists(os.getcwd() + "/numpy_arrays")) and doSave:
      os.mkdir("numpy_arrays")
    
    if(filesToDo[fileA].rpartition(".")[0] + ".png" in allIn or
       filesToDo[fileA].rpartition(".")[0] + "_fAx.png" in allIn or
       filesToDo[fileA].rpartition(".")[0] + ".npy" in allIn):# or
       # filesToDo[fileA].rpartition(".")[0] + ".npy" in os.listdir(pathToData +\
       #   "numpy_arrays")):
      #tryUntilGood(outName.rpartition('\\')[0] + "\\",\
  ...
  for dictItem in overlayDict.values():
    os.remove(pathToData + '/' + dictItem)
  #end for dictItem in overlayDict
  
  if wincmd and not(fromCmd):
